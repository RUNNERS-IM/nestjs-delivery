...
import { CardService } from '../services/<%= serviceFileName %>';
...
export class CardController {
  constructor(private <%= serviceName %>: CardService) {}
  @Post()
  @Auth([RoleType.USER])
  ...
  async createCard(@AuthUser() user: UserEntity, @Body() createCardDto: CreateCardDto) {
    // Create card
    const card: CardEntity = await this.<%= serviceName %>.create({ userId: user.id, ...createCardDto });
    // Return Card
  ...
  async getCards(@AuthUser() user: UserEntity) {
    // Create card
    const cards: CardEntity[] = await this.<%= serviceName %>.find({ userId: user.id });
    // Return Card
  ...
  async getCard(@AuthUser() user: UserEntity, @Param('id') id: Uuid) {
    // Create card
    const card: CardEntity = await this.<%= serviceName %>.findOneOrFail({
      userId: user.id,
      id: id,
    });
    // Return Card
  ...
  @ApiOperation({
    summary: '카드 삭제 API',
    description: '카드 삭제하는 API 입니다.',
  })
  @ApiOkResponse({
    description: '카드 삭제 성공시 결과 예시',
    type: CardDeleteResponse,
  })
  async deleteCard(@AuthUser() user: UserEntity, @Param('id') id: Uuid) {
    // Get card
    const card: CardEntity = await this.<%= serviceName %>.findOneOrFail({
      userId: user.id,
      id: id,
