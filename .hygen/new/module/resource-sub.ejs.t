---
to: "src/modules/<%= h.filename(name) %>/admins/<%= h.resourceFileName(subname) %>.ts"
unless_exists: true
---
<%
  // Class
...
// Option
import { <%= subResourceOptionsName %> } from './options/<%= subResourceOptionsFileName %>';

// Main section
export const <%= subResourceName %> = {
  resource: <%= SubEntityName %>,
  options: <%= subResourceOptionsName %>,
};
