---
to: "src/modules/<%= h.filename(name) %>/responses/<%= h.responseFileName(subname) %>.ts"
unless_exists: true
---
<%
  // Class
...
// Main section
export class <%= SubClassName %>CreateResponse extends CreateResponse<<%= SubEntityName %>> {
  @ApiPropertyOptional({ type: <%= SubEntityName %> })
  data?: <%= SubEntityName %>; // 결과 데이터
}
export class <%= SubClassName %>ListResponse extends ListResponse<<%= SubEntityName %>> {
  @ApiPropertyOptional({ type: [<%= SubEntityName %>], isArray: true })
  data?: <%= SubEntityName %>[]; // 결과 데이터
}
export class <%= SubClassName %>RetrieveResponse extends RetrieveResponse<<%= SubEntityName %>> {
  @ApiPropertyOptional({ type: <%= SubEntityName %> })
  data?: <%= SubEntityName %>; // 결과 데이터
}
export class <%= SubClassName %>UpdateResponse extends UpdateResponse<<%= SubEntityName %>> {
  @ApiPropertyOptional({ type: <%= SubEntityName %> })
  data?: <%= SubEntityName %>; // 결과 데이터
}
export class <%= SubClassName %>DeleteResponse extends DeleteResponse<<%= SubEntityName %>> {
  @ApiPropertyOptional({ type: <%= SubEntityName %> })
  data?: <%= SubEntityName %>; // 결과 데이터
}
