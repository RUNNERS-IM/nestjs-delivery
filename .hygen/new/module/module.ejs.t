...
import { <%= SubControllerName %> } from './controllers/<%= subControllerFileName %>';

// Repository
import { <%= RepositoryName %> } from './repositories/<%= repositoryFileName %>';
import { <%= SubRepositoryName %> } from './repositories/<%= subRepositoryFileName %>';

// Service
import { <%= ServiceName %> } from './services/<%= serviceFileName %>';
import { <%= SubServiceName %> } from './services/<%= subServiceFileName %>';

// Subscriber
import { <%= SubscriberName %> } from './subscribers/<%= subscriberFileName %>';
import { <%= SubSubscriberName %> } from './subscribers/<%= subSubscriberFileName %>';

// Main section
@Module({
  imports: [TypeOrmModule.forFeature([<%= RepositoryName %>, <%= SubRepositoryName %>])],
  controllers: [<%= ControllerName %>, <%= SubControllerName %>],
  exports: [<%= ServiceName %>, <%= SubServiceName %>],
  providers: [<%= ServiceName %>, <%= SubServiceName %>, <%= SubscriberName %>, <%= SubSubscriberName %>],
})
