---
to: "src/modules/<%= h.filename(name) %>/subscribers/<%= h.subscriberFileName(name) %>.ts"
unless_exists: true
---
<%
  // Class
...
@EventSubscriber()
export class <%= SubscriberName %> implements EntitySubscriberInterface<<%= EntityName %>> {
  constructor(
  ...
  // Listener section
  beforeInsert(event: InsertEvent<<%= EntityName %>>) {}

  beforeUpdate(event: UpdateEvent<<%= EntityName %>>) {}

  afterInsert(event: InsertEvent<<%= EntityName %>>) {}

  afterUpdate(event: UpdateEvent<<%= EntityName %>>) {}
}
