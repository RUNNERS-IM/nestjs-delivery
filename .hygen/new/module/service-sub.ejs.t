---
to: "src/modules/<%= h.filename(name) %>/services/<%= h.serviceFileName(subname) %>.ts"
unless_exists: true
---
<%
  // Class
...
// Main section
@Injectable()
export class <%= SubServiceName %> extends CrudService<<%= SubEntityName %>> {
  constructor(
    @InjectRepository(<%= SubRepositoryName %>)
    private <%= subRepositoryName %>: <%= SubRepositoryName %>,
  ) {
    super( <%= subRepositoryName %> );
  }
}
