---
to: "src/modules/<%= h.filename(name) %>/repositories/<%= h.repositoryFileName(subname) %>.ts"
unless_exists: true
---
<%
  // Class
...
// Main section
@EntityRepository(<%= SubEntityName %>)
export class <%= SubRepositoryName %> extends Repository<<%= SubEntityName %>> {}
